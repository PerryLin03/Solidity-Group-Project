...
contract EnhancedLendingProtocol is ReentrancyGuard, Ownable {
    ...
    address public constant USDT_USD_PRICE_FEED = 0xa82486565B8DCE64BFfaF1264BbC6197fb56fe9c; //這個沒找到二次確認
    ...
    function deposit(address asset, uint256 amount) external payable nonReentrant {
        require(isAssetSupported(asset), "Asset not supported for deposit"); // 檢查資產是否支持
        require(amount > 0, "Deposit amount must be greater than 0");        // 確保存款金額大於 0

        updateInterest(asset); // 更新資產利息
        ...
        if (asset == ETH) {
            ...
            require(token.allowance(msg.sender, address(this)) >= amount, "Insufficient allowance"); // 確保有足夠授權
            require(token.transferFrom(msg.sender, address(this), amount), "Transfer failed"); // 執行代幣轉移
        }

        userDeposits[asset][msg.sender].amount += amount; // 更新存款
        userDeposits[asset][msg.sender].lastUpdateTimestamp = block.timestamp; // 更新時間戳
        assetInfo[asset].totalDeposits += amount; // 更新資產存款總量

        emit Deposit(msg.sender, asset, amount); // 發送事件
    }
    ...
    // 獲取資產價值的輔助函數
    function getCollateralValue(address collateralAsset, address user) public view returns (uint256) {
        uint256 collateralAmount = userDeposits[collateralAsset][user].amount;
        return getAssetValue(collateralAsset, collateralAmount); // 計算抵押資產的價值
    }
